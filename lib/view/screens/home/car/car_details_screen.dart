  ...
  Widget build(BuildContext context) {
    return BaseWidget<CarDetailsViewModel>(
      initState: (model) {
        ...
        model.getSubscriptionOptions(carService?.id ?? -1);
      },
...
class CarDetailsViewModel extends BaseNotifier {
  ...
  Future<void> getSubscriptionOptions(int carSerId) async {
    setBusy();
    var res = await carsRepo.getSubscriptionOptions(
      queryParams: {
        "offset": "0",
        "limit": "1000",
        "car_service": "$carSerId",
      },
    );
    if (res.left != null) {
      ...
      for (var item in subscriptionOptions?.results ?? <SubscriptionOptionModel>[]) {
        switch (item.type) {
          case "RIDE":
            {
              plansList.add("RIDE");
          ...
          case "TRAVEL":
            {
              plansList.add("TRAVEL");
            }
          case "AIRPORT":
            {
              plansList.add("AIRPORT");
            }
        }
      }
      setIdle();
